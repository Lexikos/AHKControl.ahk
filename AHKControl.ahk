...
IconSize := Ceil(16*(A_ScreenDPI/90))
...
DllCall("SetThreadDpiAwarenessContext", "int", -4) ; DPI_AWARENESS_CONTEXT_PER_MONITOR_AWARE_V2
...
Process, Exist
this_pid := ErrorLevel
this_id := A_ScriptHwnd

; Load standard status icons.
this_exe := A_IsCompiled ? A_ScriptFullPath : A_AhkPath
SuspendedIcon       := "HICON:*" LoadPicture(this_exe, "Icon3", image_type)
PausedIcon          := "HICON:*" LoadPicture(this_exe, "Icon4", image_type)
PausedSuspendedIcon := "HICON:*" LoadPicture(this_exe, "Icon5", image_type)
...
if IconFile
{
    ...
    if this_icon := LoadPicture(IconFile, "Icon" IconNumber " w" IconSize, image_type)
...
ExitAll:
    if Script_Count > 1
    {
        ...
        ; Don't use this, since some scripts are excluded from the array:
        ; WinWaitClose ahk_class AutoHotkey,, 2, %A_ScriptFullPath%
        Loop, %Script_Count%
            if WinExist("ahk_id " Script_%A_Index%) && Script_%A_Index% != A_ScriptHwnd {
...
AHKControlActive:
    ...
    ; Just show the top-most script (starting a script or activating its
    ; main window puts it at the top of the z-order).
    KeyWait, LWin
    RebuildAndShowMenu(WinExist("ahk_class AutoHotkey"))
    return
...
{
    local i, id, pid, title, pos, hasTrayIcon, filename
        , h_icon, width, height, new_bitmap, ext, _, menu_item_text
    ...
    Loop, %ahk_list%
    {
        ...
        ; if (InStr(script_title, "\toolbar\Toolbar.ahk - ")) ; HACK: Ignore SciTE toolbar.
            ; continue
        ...
        if !(h_icon || script_is_hung)
        {
            ...
            ; OutputDebug WM_GETICON %ErrorLevel%
            if ErrorLevel
                h_icon := ErrorLevel
        }
        ...
        if h_icon
        {
            icon_file := "HICON:*" h_icon
        }
        else
        {
            if (ext = "exe")
                icon_file := script_path
            
            ; Show standard Paused, Suspended, or Paused/Suspended icon where applicable.
            if isPaused && isSuspended
                icon_file := PausedSuspendedIcon
            else if isPaused
                icon_file := PausedIcon
            else if isSuspended
                icon_file := SuspendedIcon
        }
        ; Set menu item icon
        if icon_file
            Menu, Root, Icon, % i "&", % icon_file
        i += 1
    }
